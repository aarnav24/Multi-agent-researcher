...
#         key_idx = _gemini_key_index - 1
#     if not key:
#         raise RuntimeError("No GOOGLE_API_KEY configured in .env")
#     llm = ChatGoogleGenerativeAI(model=model, api_key=key, temperature=temperature, max_retries=0)
#     if return_key_idx:
#         return llm, key_idx
#     return llm
...
# ═══════════════════════════════════════════════════════════════════════════════
# PROVIDER: GROQ (commented out — uncomment to switch back)
# ═══════════════════════════════════════════════════════════════════════════════

# def _get_groq_llm(model: str, temperature: float = 0.0, api_key: str | None = None,
#                   return_key_idx: bool = False):
...
#     else:
#         key = _next_groq_key()
#         key_idx = _groq_key_index - 1
#     if not key:
#         raise RuntimeError("No GROQ_API_KEY configured in .env")
#     llm = ChatGroq(model=model, api_key=key, temperature=temperature, max_retries=0)
#     if return_key_idx:
#         return llm, key_idx
#     return llm
#
...
#     global _groq_key_cycle, _groq_key_index
...
def _next_openrouter_key() -> str:
    ...
    if _or_key_cycle is None:
        ...
        if not keys:
            raise RuntimeError("No OPENROUTER_KEY configured in .env")
...
def get_llm(model: str, temperature: float = 0.0, api_key: str | None = None,
    ...
    Auto-detects provider from model name:
      ...
      - Others → uses the configured LLM_PROVIDER (groq/openrouter)
    ...
    if provider == "groq":
        return _get_groq_llm(model, temperature=temperature, api_key=api_key,
                             return_key_idx=return_key_idx)
    elif provider == "openrouter":
        return _get_openrouter_llm(model, temperature=temperature, api_key=api_key,
                                   return_key_idx=return_key_idx)
    else:
        raise RuntimeError(f"Unknown LLM_PROVIDER: {provider}. Use 'groq' or 'openrouter'.")
...
def get_reasoning_llm():
    """Reasoning-tier agents use gemini-3.5-flash as primary.

    All reasoning agents (Orchestrator, Planner, Synthesizer, Critic) use this.
    Falls back to gpt-oss-120b → gemini-2.5-flash → openrouter/auto.
    """
    return get_llm(settings.model_reasoning)


def get_fast_llm():
    """Fast/cheap model for Searchers, Browsers, Citation Formatter, FactChecker, Critic.

    Uses OpenRouter auto-routing (picks best available free model).
    Falls back to gemini-2.5-flash → llama-3.3-70b.
    """
    return get_llm(settings.model_fast)
