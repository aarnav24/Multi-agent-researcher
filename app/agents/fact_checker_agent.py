...
from app.tools.tavily_tool import tavily_search, get_tavily_answer
from app.tools.exa_tool import exa_search
from app.tools.ddg_tool import ddg_search
from app.tools.query_normalizer import normalize_query
from app.agents.searcher_agent import _call_tool_rate_limited
...
class FactCheckerAgent(BaseAgent):
    ...
    # FactChecker uses fast tier (llama-4-scout) as primary — structured verification
    # doesn't need deep reasoning. Gemini is fallback if Groq hits rate limits.
    @property
    def llm(self):
    ...
    async def verify_claim_with_evidence(
        ...
        """
        # Capture sources + tavily answer during verification by re-running the
        # search portion manually. To keep things simple and DRY, we duplicate
        # only the search calls here and then call the existing LLM-judgment path.
        import asyncio
        from app.tools.tavily_tool import tavily_search, get_tavily_answer
        from app.tools.exa_tool import exa_search
        from app.tools.ddg_tool import ddg_search
        ...
        # Per-agent call counter for this verification pass
        call_counter = {"count": 0}
        ...
        result = await self.verify_claim(claim, browser_facts=browser_facts)
