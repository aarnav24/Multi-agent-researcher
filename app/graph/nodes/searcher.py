...
from app.utils.concurrency import agent_slot, check_kill_switches
...
async def searcher_dispatch_node(state: ResearchGraphState, config: RunnableConfig | None = None) -> ResearchGraphState:
    ...
    tasks = [_run_single_searcher(sq, stagger_delay=i * 1.0, sse=sse) for i, sq in enumerate(pending)]
    results = await asyncio.gather(*tasks, return_exceptions=True)

    # Emit agent_status completion for each searcher worker
    if sse:
