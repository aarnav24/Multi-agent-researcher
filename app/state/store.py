...
from app.state.models import AuditEntry, ResearchSession
...
class StateStore:
    ...
    def _coerce_claims(session: ResearchSession, data: Any) -> None:
        from app.state.models import VerifiedClaim, RejectedClaim
    ...
    def _coerce_sub_questions(session: ResearchSession, data: Any) -> None:
        from app.state.models import SubQuestion
    ...
    def _coerce_verified_claims(session: ResearchSession, data: Any) -> None:
        from app.state.models import VerifiedClaim
        if isinstance(data, list):
            session.verified_claims = [
                VerifiedClaim(**c) if isinstance(c, dict) else c
                for c in data
    ...
    def _coerce_rejected_claims(session: ResearchSession, data: Any) -> None:
        from app.state.models import RejectedClaim
        if isinstance(data, list):
            session.rejected_claims = [
                RejectedClaim(**c) if isinstance(c, dict) else c
                for c in data
    ...
    async def connect(self):
        ...
        try:
            from app.citation_graph.neo4j_adapter import Neo4jAdapter, connect as neo4j_connect
    ...
    async def disconnect(self):
        ...
        if self._neo4j and self._neo4j.enabled:
            from app.citation_graph.neo4j_adapter import disconnect as neo4j_disconnect
