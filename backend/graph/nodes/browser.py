...
async def _run_single_browser_with_pool(
    ...
    async with _browser_semaphore:
        return await _run_single_browser(url, research_question, 0, user_id=user_id)
...
async def _run_single_browser(
    ...
    async with agent_slot():
        agent = BrowserAgent(user_id=user_id)
...
async def browser_dispatch_node(state: ResearchGraphState, config: RunnableConfig | None = None) -> ResearchGraphState:
    ...
    if sse:
        sse.emit("agent_start", {
            "agent": "browsers",
            "message": "Browsers deep-fetching pages",
            "tier": "fast",
            "model": "browsers",
        })
    ...
    for i, result in enumerate(results):
        if isinstance(result, Exception):
            ...
            continue
        # Accumulate tool calls from this browser worker
